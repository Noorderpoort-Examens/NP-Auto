    ...
    <div x-data="{ selectedImage: '{{ asset('storage/' . ($images[0] ?? 'img/fallback-image.png')) }}' }" class="max-w-4xl mx-auto bg-white shadow-lg my-3 lg:my-5 overflow-hidden rounded-lg">
        <div class="flex flex-col md:flex-row overflow-hidden mx-3 mb-2 md:min-h-80 my-2">
            <div class="w-full md:w-2/3 md:max-h-80">
                @if ($images)
                    <img :src="selectedImage" alt="{{ $occasion->brand }} {{ $occasion->model }}"
                        class="object-cover w-full h-full rounded">
                @else
                    <img src="{{ asset('img/fallback-image.png') }}" alt="{{ $occasion->brand }} {{ $occasion->model }}"
                        class="object-contain bg-np-color-shade w-full h-full rounded">
                @endif
            </div>

            <div class="w-full md:w-2/3 px-6 py-3 flex flex-col justify-between">
                <div>
                    <h2 class="font-roboto text-lg md:text-xl lg:text-2xl font-bold mb-1">
                        {{ $occasion->advertisingtitle }}</h2>
                    <h3 class="font-opensans text-sm md:text-base lg:text-lg mb-1">Prijs:
                        €{{ number_format($occasion->askprice, 2, ',', '.') }}</h3>
                    <p class="font-opensans text-xs md:text-sm lg:text-base text-gray-700 mb-1">
                        {{ $occasion->description }}</p>
                </div>

                @if ($occasion->sold)
                    <div
                        class="font-roboto mt-5 text-center w-full rounded-lg bg-red-600 px-5 py-2.5 text-sm md:text-base lg:text-lg font-medium text-white">
                        Verkocht
                    </div>
                @elseif ($occasion->reserved)
                    <div
                        class="font-roboto mt-5 text-center w-full rounded-lg bg-np-yellow px-5 py-2.5 text-sm md:text-base lg:text-lg font-medium text-dark">
                        Gereserveerd
                    </div>
                @else
                    <button type="button"
                        class="font-roboto mt-5 w-full rounded-lg bg-np-dark px-5 py-2.5 text-sm md:text-base lg:text-lg font-medium text-white hover:bg-np-yellow/90 focus:outline-none focus:ring-4 focus:ring-np-white transition">
                        Reserveren
                    </button>
                @endif
            </div>
